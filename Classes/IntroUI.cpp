...
void IntroUI::SetStart(Scene * pScene) {
  _start = Sprite::create("Intro/UI/Start.png");
  _start->setAnchorPoint(Vec2(0, 0));
  _start->setPosition(Vec2(372, 175));
  pScene->addChild(_start);
}

void IntroUI::SetDelete(Scene * pScene) {
  _delete = Sprite::create("Intro/UI/Delete.png");
  _delete->setAnchorPoint(Vec2(0, 0));
  _delete->setPosition(Vec2(372, 130));
  pScene->addChild(_delete);
}
...
void IntroUI::SetBtnCatUp(Scene * pScene) {
  _btnCatUp = Sprite::createWithSpriteFrameName("btn_cat_up.png");
  _btnCatUp->setAnchorPoint(Vec2(0, 0));
  _btnCatUp->setPosition(Vec2(0, 0));
  pScene->addChild(_btnCatUp);
}
